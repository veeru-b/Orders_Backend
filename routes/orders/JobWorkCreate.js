...
const insertIntoCombinedSchedule = async (custCode) => {
  const result = await mchQueryMod1(
    "INSERT INTO magodmis.combined_schedule (Cust_code) VALUES (?)",
    [custCode]
  );
...
const getCountOfCombinedScheduleDetails = async (cmbSchId) => {
  const result = await mchQueryMod1(
    `SELECT COUNT(*) AS rowCont FROM magodmis.combined_schedule_details WHERE cmbSchId = ?`,
    [cmbSchId]
  );
...
const updateOrderscheduleAndNCTaskList = async (
  ...
  try {
    ...
    await mchQueryMod1(
      ...
      WHERE o.ScheduleID = ?`,
      ["Comb/" + combinedScheduleNo, scheduleId]
    );
